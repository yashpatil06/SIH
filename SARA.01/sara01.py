...
say("Hi ! Sara HERE from HealthByte Innovators !!")
    ...
    try:
        ...
        for proc in psutil.process_iter(['pid', 'name']):
            ...
            if ("word" in app_name and "winword" in process_name) or \
                    ...
                    ("power point" in app_name or "powerpoint" in  app_name and "ppt" in process_name) or \
                ...
                proc.kill()  # Try using kill() instead of terminate()
        ...
        if not found:
            say(f"Sorry, I can't find {app_name} to close.")
        elif not process_closed:
            say(f"Could not close {app_name}. It may already be closed.")
...
while True:
    ...
    if text:
        ...
        if "conclusion" in text.lower() or "conclude" in text.lower():
            say("Voice Assistant technology can truly revolutionize accessibility and help "
                "millions of disabled individuals achieve greater independence. "
                "By providing hands-free voice control of their systems, "
                "speech recognition technology opens up a new way for human interaction with machines, "
                "greatly benefiting physically challenged individuals")
